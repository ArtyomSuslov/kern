...
dump_virtual_tree(struct Page *node, int class) {
    ...
    // Если узел дерева отсутствует, завершить выполнение функции.
    if (!node) {
        return;
    }

    // Отступы для визуализации глубины дерева.
    for (int i = MAX_CLASS; i != class; --i) {
        cprintf(" ");  // Каждому уровню дерева соответствует пробел
    }

    // Выводим информацию о типе узла
    if ((node->state & NODE_TYPE_MASK) == MAPPING_NODE) {
        cprintf("Mapping to addr: %lx, class: %d\n", 
                (long unsigned)node->phy->addr, node->phy->class);
    } else {
        cprintf("Intermediate page, class: %d\n", class);
    }

    // Рекурсивный вызов для левого дочернего узла с уменьшением класса
    if (node->left) {
        cprintf("L- ");
        dump_virtual_tree(node->left, class - 1);
    }

    // Рекурсивный вызов для правого дочернего узла с уменьшением класса
    if (node->right) {
        cprintf("R- ");
...
dump_page_table(pte_t *pml4) {
    ...
    // Перебираем все записи в таблице PML4 (Page Map Level 4)
    for (int i = 0; i < PML4_ENTRY_COUNT; ++i) {
        // Пропускаем записи, которые не присутствуют (флаг PTE_P не установлен)
        if (!(pml4[i] & PTE_P)) {
            continue;
        }

        // Выводим информацию о текущей записи в PML4
        dump_entry(pml4[i], 512 * GB, 0);

        // Получаем указатель на таблицу PDPT (Page Directory Pointer Table)
        pdpe_t *pdp = KADDR(PTE_ADDR(pml4[i]));

        // Перебираем все записи в PDPT
        for (int j = 0; j < PDP_ENTRY_COUNT; ++j) {
            // Пропускаем записи, которые не присутствуют или представляют 1 ГБ страницы (PTE_PS установлен)
            if (!(pdp[j] & PTE_P && !(pdp[j] & PTE_PS))) {
                continue;
            }

            // Выводим информацию о текущей записи в PDPT
            dump_entry(pdp[j], 1 * GB, pdp[j] & PTE_PS);

            // Получаем указатель на таблицу PD (Page Directory)
            pde_t *pd = KADDR(PTE_ADDR(pdp[j]));

            // Перебираем все записи в PD
            for (int k = 0; k < PD_ENTRY_COUNT; ++k) {
                // Пропускаем записи, которые не присутствуют или представляют 2 МБ страницы (PTE_PS установлен)
                if (!(pd[k] & PTE_P && !(pd[k] & PTE_PS))) {
                    continue;
                }

                // Выводим информацию о текущей записи в PD
                dump_entry(pd[k], 2 * MB, pd[k] & PTE_PS);

                // Получаем указатель на таблицу PT (Page Table)
                pte_t *pt = KADDR(PTE_ADDR(pd[k]));

                // Перебираем все записи в PT
                for (int l = 0; l < PT_ENTRY_COUNT; ++l) {
                    ...
                    // Выводим информацию о текущей записи в PT
...
memcpy_page(struct AddressSpace *dst, uintptr_t va, struct Page *page) {
    ...
    // Переключаем адресное пространство на целевое, чтобы получить доступ к виртуальному адресу va
    struct AddressSpace *old = switch_address_space(dst);

    // Отключаем защиту записи (Write Protect) в CR0
    set_wp(0);

    // Выполняем копирование данных
    // Источник:   физический адрес страницы, преобразованный в виртуальный адрес ядра через KADDR
    // Назначение: виртуальный адрес va в целевом адресном пространстве
    // Размер копируемых данных определяется классом страницы CLASS_SIZE(page->class)
    nosan_memcpy((void *)va, KADDR(page2pa(page)), CLASS_SIZE(page->class));

    // Включаем защиту записи обратно для обеспечения целостности и безопасности системы.
    set_wp(1);

    // Возвращаемся к исходному адресному пространству.
...
unmap_page(struct AddressSpace *spc, uintptr_t addr, int class) {
    ...
    // Вычисляем индексы PD для начала и конца диапазона.
    size_t pdi0 = PD_INDEX(addr);
    size_t pdi1 = PD_INDEX(end);

    // Если pdi0 > pdi1 (разрыв индексов из-за границы), корректируем pdi1.
    if (pdi0 > pdi1) {
        pdi1 = PD_ENTRY_COUNT;
    }

    // Если класс >= 9 (2 МБ), обрабатываем таблицу PD.
    if (class >= 9) {
    ...
    // Проверяем, что запись PD присутствует. Если отсутствует, выходим.
    if (!(pd[pdi0] & PTE_P)) {
        return;
    } else if (pd[pdi0] & PTE_PS) /* Если запись PD представляет 2 МБ страницу, делим её на 4 КБ страницы */ {
        
        // Сохраняем старое значение записи в PD, чтобы позже восстановить его без флага PTE_PS
        pde_t old = pd[pdi0];

        // Выделяем новую таблицу страниц (PT) для текущей записи в PD
        res = alloc_pt(pd + pdi0);
        assert(!res); // Убедимся, что выделение прошло без ошибок

        // Получаем указатель на новую таблицу страниц (PT) из физического адреса записи в PD.
        pte_t *pt = KADDR(PTE_ADDR(pd[pdi0]));

        // Разбиваем запись на несколько 4 КБ страниц, используя старые флаги, но без флага PTE_PS
        // Функция alloc_fill_pt заполняет таблицу страниц с нужными флагами и выделяет необходимые записи
        res = alloc_fill_pt(pt, old & ~PTE_PS, 4 * KB, 0, PT_ENTRY_COUNT);

        // Округляем диапазон для инвалидации TLB до границ 2 МБ страниц.
        inval_start = ROUNDDOWN(inval_start, 2 * MB);
        inval_end = ROUNDUP(inval_end, 2 * MB);

        assert(!res); // Убедимся, что операция прошла успешно
    }

    // Загружаем указатель на таблицу PT (Page Table).
    pte_t *pt = KADDR(PTE_ADDR(pd[pdi0]));
...
map_page(struct AddressSpace *spc, uintptr_t addr, struct Page *page, int flags) {
    ...
    // Вычисляем индексы в таблице PD для начала и конца диапазона
    size_t pdi0 = PD_INDEX(addr);
    size_t pdi1 = PD_INDEX(end);

    // Корректируем pdi1, если разрыв индексов
    if (pdi0 > pdi1) {
        pdi1 = PD_ENTRY_COUNT;
    }

    // Если размер страницы >= 2 МБ, заполняем диапазон в PD
    if (page->class >= 9) {
    ...
    // Проверяем, есть ли валидная запись в PD, если нет — выделяем новую таблицу страниц
    if (!(pd[pdi0] & PTE_P) && alloc_pt(pd + pdi0) < 0) {
        return -E_NO_MEM;
    } else if (pd[pdi0] & PTE_PS) { 
        // Если запись в PD указывает на 2 МБ страницу, разбиваем её на 4 КБ страницы
        pde_t old = pd[pdi0];
        ...
        pte_t *pt = KADDR(PTE_ADDR(pd[pdi0]));
        // Заполняем таблицу страниц для 4 КБ
        if (alloc_fill_pt(pt, old & ~PTE_PS, 4 * KB, 0, PT_ENTRY_COUNT) < 0) {
            return -E_NO_MEM;
        }
    }

    // Получаем указатель на таблицу страниц (PT)
    pte_t *pt = KADDR(PTE_ADDR(pd[pdi0]));
...
switch_address_space(struct AddressSpace *space) {
    ...
    // Если текущее адресное пространство уже равно переданному, ничего менять не нужно
    if (space == current_space) {
        return current_space;
    }

    // Сохраняем текущее адресное пространство, чтобы вернуться к нему позже
    struct AddressSpace *old_space = current_space;

    // Переключаем на новое адресное пространство
    current_space = space;

    // Загружаем значение регистра CR3 для нового адресного пространства
    // CR3 указывает на корневую таблицу страниц (PML4) для данного адресного пространства
    lcr3(current_space->cr3);

    // Возвращаем предыдущее адресное пространство
    return old_space;
}
...
/*
 * Обёртка для функции map_physical_region, которая проверяет возвращаемое значение и
 * если нужно, паникует
*/
#define MAP_KERNEL_REGION(dstart, pstart, size, flags)                                               \
    do {                                                                                             \
        if (map_physical_region(&kspace, (dstart), (pstart), (size), (flags)) < 0) {                 \
            panic("Cannot map physical region at %p of size %zd", (void *)(pstart), (size_t)(size)); \
        }                                                                                            \
    } while (0)
...
init_memory(void) {
    ...
    // Map [0, max_memory_map_addr] to 
    //     [KERN_BASE_ADDR, KERN_BASE_ADDR + max_memory_map_addr] as RW- + ALLOC_WEAK
    ...
    MAP_KERNEL_REGION((uintptr_t)__text_start, PADDR(__text_start), 
                      __text_end - __text_start, PROT_RWX);

    /* Allocate kernel stacks */

    // LAB 7: Your code here
    // Map [PADDR(bootstack), PADDR(bootstack) + KERN_STACK_SIZE] to 
    //     [KERN_STACK_TOP - KERN_STACK_SIZE, KERN_STACK_TOP] as RW-
    MAP_KERNEL_REGION(KERN_STACK_TOP - KERN_STACK_SIZE, PADDR(bootstack), 
                      KERN_STACK_SIZE, PROT_R | PROT_W);

    // Map [PADDR(pfstack), PADDR(pfstack) + KERN_PF_STACK_SIZE] to 
    //     [KERN_PF_STACK_TOP - KERN_PF_STACK_SIZE, KERN_PF_STACK_TOP] as RW-
    MAP_KERNEL_REGION(KERN_PF_STACK_TOP - KERN_PF_STACK_SIZE, 
    ...
    for (; mstart < mend; mstart = (void *)((uint8_t *)mstart + uefi_lp->MemoryMapDescriptorSize)) {
        if (mstart->Attribute & EFI_MEMORY_RUNTIME) {
            ...
            MAP_KERNEL_REGION(mstart->VirtualStart, mstart->PhysicalStart, 
    ...
    MAP_KERNEL_REGION(FRAMEBUFFER, uefi_lp->FrameBufferBase, uefi_lp->FrameBufferSize, 
    ...
    MAP_KERNEL_REGION(X86ADDR(KERN_BASE_ADDR), 0, MIN(MAX_LOW_ADDR_KERN_SIZE, 
